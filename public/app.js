...
// 記事件数に応じてノード半径をスケーリングする（線形スケール）
// subtopic: count=3→10, count=10→17, count=20→27, count=30→37→cap30
// topic:    count=10→18, count=30→25, count=60→35, count=100→49→cap42
function nodeRadius(d) {
  if (d.type === 'center')  return CENTER_RADIUS;
  if (d.type === 'article') return ARTICLE_RADIUS;
  if (d.type === 'topic')   return Math.min(42, Math.max(18, 14 + d.childCount * 0.35));
  // subtopic
  return Math.min(30, Math.max(10, 7 + d.childCount * 1.0));
}
